...
class FullyConnectedNN(Model):
    ...
    def train1(self, x, y, epochs=1, batch_size=1, train_test_split=0.9):
        """Train
    ...
    def train(self, x, y, epochs=1, batch_size=1, train_test_split=0.9):
        ...
        #optimizer = DistributionallyRobustOptimizer()
        ...
        for ep in range(0, epochs):
            ...
            epoch_accuracy = 0
            epoch_train_accuracy = 0
            #optimizer.optimize(x_train, y_train, self)
            ...
            while i < (total_data_size // batch_size) and (i+1)*batch_size < y_train.shape[1]:
                ...
                for layer in reversed(self.model):
                    ...
                    if curr_layer_index == last_layer_index:
                        ...
                        #Get orior layer
                        prior_hidden_layer = self.model[curr_layer_index-1]
                        ...
                        layer.delta = delta
                        layer.weights = layer.weights + self.learning_rate*np.dot(delta,prior_output_a.T)*(1/y_batch.shape[1]) - self.reg_lambda*grad_penalty_term*(1/y_batch.shape[1])
                    ...
                    #Check of this is the input hidden layer
                        ...
                        next_hidden_layer = self.model[curr_layer_index+1]
                        ...
                        layer.delta = delta
                        layer.weights = layer.weights + self.learning_rate*np.dot(delta,x_batch.T)*(1/prior_output_a.shape[1]) - self.reg_lambda*grad_penalty_term*(1/y_batch.shape[1])
                        layer.bias = layer.bias + self.learning_rate*np.mean(delta)
                    else:
                        ...
                        prev_hidden_layer = self.model[curr_layer_index-1]
                        ...
                        layer.delta = delta
                        layer.weights = layer.weights + self.learning_rate*np.dot(delta,prev_layer_output.T)*(1/prior_output_a.shape[1]) - self.reg_lambda*grad_penalty_term*(1/y_batch.shape[1])
                        layer.bias = layer.bias + self.learning_rate*np.mean(delta)
                    ...
                    curr_layer_index -= 1
                    #print(loss_penalty_term,grad_penalty_term)
...
if __name__ == '__main__':
    ...
    if train:
        ...
        model.train1(x,y,epochs=100, batch_size=8, train_test_split=0.8)
